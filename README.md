...
이 프로젝트의 목적은 단순히 API 데이터를 가져오는 것이 아니라, 원천별 수집 주기와 장애 복구 특성을 반영해 `raw -> bronze -> silver -> mart -> serving` 흐름을 설계하고, DuckDB 기반 mart의 동시 쓰기 문제를 Airflow orchestration 관점에서 해결하는 것입니다.

![Pipeline Architecture](docs/image/pipeline-architecture.png)

## 핵심 요약

- `Apache Airflow`로 KIS 주가, MK RSS, OpenDART 공시 수집 DAG를 구성했습니다.
- 수집 DAG는 원천 데이터를 보존하고 silver 파일과 생산 manifest까지만 생성합니다.
- 별도 mart 적재 DAG가 manifest를 읽어 아직 적재되지 않은 silver 파일만 DuckDB mart에 반영합니다.
...
- FastAPI 웹 앱은 DuckDB serving view를 읽어 가격 타임라인과 연결 이벤트를 제공합니다.
...
| `collect_kis_stock_price_daily_history_raw` | 평일 19:00, catchup 가능   | 일봉 이력 수집과 mart 적재                            |
...
수집 DAG가 DuckDB mart까지 직접 쓰면 source별 DAG가 서로 같은 `.duckdb` 파일에 동시에 접근할 수 있습니다. 이 프로젝트는 수집 DAG의 책임을 silver 파일 생성까지로 제한하고, mart 반영은 `load_silver_to_mart` DAG가 전담하게 분리했습니다.
...
KIS 현재가는 장중 1분 snapshot, MK RSS는 10분 polling, 일봉 이력은 날짜 단위 backfill 대상으로 다룹니다. 원천의 생성 주기와 복구 가능성이 다르기 때문에 DAG schedule, `catchup`, `execution_timeout`, `max_active_runs`를 source별로 다르게 설정했습니다.
...
FastAPI 앱은 DuckDB serving view를 읽어 종가 흐름과 연결 이벤트를 제공합니다.
...
KIS 현재가 수집을 실행하려면 `airflow/.env`에 KIS Open API 인증 정보가 필요합니다.
...
현재 코드의 기본 수집 대상 종목은 `LX세미콘(108320)`입니다.
...
웹 앱은 `../airflow/s3/mart/stock_signal.duckdb`를 read-only로 조회합니다. mart 파일이 아직 없거나 serving view가 없으면 샘플 또는 빈 데이터가 표시될 수 있습니다.
...
├── airflow
│   ├── dags                 # Airflow DAG 정의
│   ├── plugins              # 수집, 변환, mart 적재 로직
│   ├── s3                   # 로컬 S3-style 데이터 레이크
│   └── docker-compose.yaml
├── docs
...
- 짧은 장애와 긴 장애를 나눠 복구할 수 있도록 자동 lookback 적재와 수동 recovery DAG를 함께 설계했습니다.
...
- Airflow task log 외의 운영용 metric, alerting, SLA 관리는 아직 없습니다.
